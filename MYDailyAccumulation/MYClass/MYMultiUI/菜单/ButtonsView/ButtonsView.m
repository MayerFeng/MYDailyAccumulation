...
+ (ButtonsView *)btnsViewWithFrame:(CGRect)frame
    ...
    ButtonsView *btnsView = nil;
    if (type == BtnsViewTypeSelect) {
        
        btnsView = [[ButtonsView alloc] initWithFrame:frame btnTitles:btnTitles titleColor:kkColor_segment_titleNormal selectedColor:kkColorMain];
        btnsView.backgroundColor = kColor_CustomEffectViewbg;
        btnsView.effectView.alpha = 1.0;
        btnsView.slideBarHeight = 3;
        btnsView.slideBarOffSetH = -0.5;
        btnsView.splitColor = kkColor_segment_split;
        btnsView.splitWidth = 0.8;
        btnsView.splitMargin = 10;
        [btnsView selectBtn:btnsView.btns[0]];
        
    } else {
        
        btnsView = [[ButtonsView alloc] initWithFrame:frame btnTitles:btnTitles titleColor:kDarkGrayColor];
        btnsView.backgroundColor = kWhiteColor;
        btnsView.effectView.alpha = 1.0;
        btnsView.splitWidth = 0.8;
        btnsView.splitMargin = 1;
//        [btnsView addTopLineWithColor:[UIColor colorWithWhite:0.88 alpha:1.0] height:1.0];
    }
    return btnsView;
}
//不同颜色
+ (ButtonsView *)btnsViewWithFrameAndDifferentColor:(CGRect)frame
                                     ...
                                     selectedImages:(NSArray *)selectedImages
                                             ...
                                             margin:(CGFloat)margin
                                              space:(CGFloat)space{
//     ButtonsView *btnsView = nil;
    ButtonsView *btnsView = [[ButtonsView alloc] initWithFrameWithDifferentColor:frame btnTitles:btnTitles titleColor:titleColor selectedColor:seletedColor normalImages:normalImages selectedImages:selectedImages style:style layout:layoutStyle margin:margin space:space];
//    btnsView.backgroundColor = kColor_CustomEffectViewbg;
    btnsView.effectView.alpha = 1.0;
    btnsView.slideBarHeight = 3;
    btnsView.slideBarOffSetH = -0.5;
...
//!!!!!!这是单选按钮
//自定义 图标型按钮（单选中） 初始化方法
- (instancetype)initWithFrameWithDifferentColor:(CGRect)frame
                    btnTitles:(NSArray *)btnTitles
                   titleColor:(NSArray *)titleColor
                selectedColor:(NSArray *)seletedColor
                 normalImages:(NSArray *)normalImages
...
//        [self addEffectView];   //添加毛玻璃
        ...
        for (int i = 0; i < count; i++)
        {
            QButton *btn = [[QButton alloc] initWithFrame:CGRectMake(i * btnWidth, 0, btnWidth, selfHeight) style:style layoutStyle:layoutStyle font:[UIFont systemFontOfSize:12] title:btnTitles[i] image:[UIImage imageNamed:normalImages[i]] space:space margin:margin autoSize:NO];
            [self.btns addObject:btn];
            
            [btn setTitle:btnTitles[i] forState:UIControlStateNormal];
            [btn setTitleColor:[UIColor colorWithHexString:titleColor[i]] forState:UIControlStateNormal];
            [btn setTitleColor:[UIColor colorWithHexString:seletedColor[i]] forState:UIControlStateSelected];
            [btn setImage:[UIImage imageNamed:normalImages[i]] forState:UIControlStateNormal];
            [btn setImage:[UIImage imageNamed:selectedImages[i]] forState:UIControlStateSelected];
...
- (void)addEffectView
{
    self.backgroundColor = kColor_CustomEffectViewbg;
    //创建需要的毛玻璃特效类型
    ...
    [self addBottomLineWithColor:[UIColor colorWithWhite:0.88 alpha:1.0] height:0.7];
}
...
{
    [self.delegate btsView:self clicked:sender atIndex:sender.tag - 101 isRepeated:NO];
}
...
//    self.bthBlock(sender); (代码块用于title船只)
    NSLog(@"到底为什么不想赢");
    [self.delegate btsView:self clicked:sender atIndex:sender.tag - 101 isRepeated:self.isRepeated];
}

- (void)selectBtn:(UIButton *)sender
{
    self.isRepeated = (self.lastBtn == sender);
...
//    [self selectBtn:sender];
//    self.isRepeated = (self.lastBtn == sender);
    NSLog(@"到底为什么不想赢----------点击按钮的输出");
    [self.delegate btsView:self clicked:sender atIndex:sender.tag];
