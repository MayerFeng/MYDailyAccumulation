...
- (void)layoutViews
{
    CGRect rect = [@"哈哈" boundingRectWithSize:CGSizeMake(self.bounds.size.width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:_label.font} context:nil];
    CGSize labelSize = CGSizeMake(ceilf(rect.size.width), ceilf(rect.size.height));
    
    CGFloat label_H = labelSize.height;
    CGFloat image_W_H = self.bounds.size.height - kTopMargin - kVerSpace - kButtomMargin - label_H;
    
    self.imageView.frame = CGRectMake((self.bounds.size.width - image_W_H) / 2.0, kTopMargin, image_W_H, image_W_H);
    self.label.frame = CGRectMake(0, CGRectGetMaxY(_imageView.frame) + kVerSpace, self.bounds.size.width, label_H);
}
