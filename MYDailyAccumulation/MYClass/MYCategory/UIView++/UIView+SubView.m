...
- (void)addMiddleBottomLine{
    ...
    lineView.backgroundColor = kLine_Color_Default;
    ...
    if ([self isKindOfClass:[UIScrollView class]])
    {
        if (self.frame.size.height != 0 && self.frame.size.height != 0)
        {
            ...
            lineView.frame = CGRectMake(10, self.size.height + bottomLayoutNum - 0.7, self.size.width- 2*10, 0.7);
        }
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:lineView attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeLeft multiplier:1 constant:10]];
    
    [self addConstraint:[NSLayoutConstraint constraintWithItem:lineView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeRight multiplier:1 constant:-10]];
    
    [self addConstraint:[NSLayoutConstraint constraintWithItem:lineView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:0 multiplier:1 constant:0.7]];
}
- (void)addBottomLine
{
    [self addBottomLineWithColor:kLine_Color_Default height:kLine_W_H_Default style:LineLayoutStyleMiddle];
}
...
- (void)addBottomLineWithColor:(UIColor *)color height:(CGFloat)height style:(LineLayoutStyle)style;
{
    ...
    if ([self isKindOfClass:[UIScrollView class]])
    {
        if (self.frame.size.height != 0 && self.frame.size.height != 0)
        {
            ...
            lineView.frame = CGRectMake(10, self.size.height + bottomLayoutNum - height, self.size.width- 2*10, height);
        }
